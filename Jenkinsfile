...
node {
    ...
    withCredentials([file(credentialsId: SERVER_KEY_CREDENTIALS_ID, variable: 'jwt_key_file')]) {
        stage('Authorize to Salesforce') {
			...
			rc = command "${toolbelt}/sfdx force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --jwtkeyfile /Users/damarante/Documents/certificates/server.key --username ${DEV_HUB} "
		    println rc
		    if (rc != 0) {
